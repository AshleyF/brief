...
Metacircularity if fun! Let's start writing Brief in Brief itself. Starting with the lexer and then the parser.
...
The lexer tokenizes a string of text; breaking on whitespace (except for strings in quotes), treating brackets as tokens (except when escaped), etc.
...
For example, `lex "this [is {a}] test"` -> `['test '\] '\} 'a '\{ 'is '\[ 'this]`. Brackets are split and otherwise simple tokens are broken on whitespace or brackets. Another example, `lex "this \"foo is a\" test"` -> `['test "'foo is a" 'this]`. Regular strings become a sinple token (notice, with a leading tick).

The parser then assembles this flat list of tokens into a list of structured values. Strings remain as such, but symbols and numbers are converted. Nested brackets and braces are built into list and maps.
...
Writting this code was a pretty fun exercise in using Brief to do semi-serious work. It's nice to see also that it ends up more "brief" than the F#.
